...
const UsuariosLista = () => {
  ...
  const fetchUsuarios = async () => {
    try {
      ...
      showToast('Error al cargar usuarios: ' + (err.response?.data?.message || err.message), 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    if (protectedEmails.includes(deleteModal.usuario.email.toLowerCase())) {
      showToast('No se puede eliminar este usuario. Es un usuario protegido del sistema.', 'error', 5000);
    ...
    try {
      ...
      showToast('Error al eliminar usuario: ' + (err.response?.data?.message || err.message), 'error');
      setDeleteModal({ show: false, usuario: null });
    }
  };
                  ...
                  {filteredUsuarios.length === 0 ? (
                    ...
                    filteredUsuarios.map((usuario) => (
                      <tr key={usuario.id}>
                        ...
                        <td>
                          {usuario.nombre || '-'}
                          {isProtectedUser(usuario.email) && (
                            <i className="bi bi-shield-fill-check text-warning ms-2" title="Usuario protegido"></i>
                          )}
                        </td>
                        <td>{usuario.email}</td>
                        <td>
                        ...
                        <td className="text-center table-actions">
                          {isProtectedUser(usuario.email) ? (
                            <span className="badge bg-secondary" title="Usuario protegido - No se puede eliminar">
